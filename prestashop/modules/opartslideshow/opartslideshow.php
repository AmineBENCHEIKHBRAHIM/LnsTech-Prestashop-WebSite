...
{			
  ...
  public function uninstall()
	...
	private function prepareHook($hookName) {
		...
		//recup slides
		$sqlSlide='
		SELECT s.id_opartslideshow_slideshow,s.active,s.width,s.height,s.spw,s.sph,s.delay,s.sDelay,s.opacity,s.titleSpeed,s.effect,s.navigation,s.links,s.hoverpause,s.home,sl.name
		...
		WHERE s.active=1 AND hook="'.$hookName.'" AND sl.id_lang = '.(int)$this->context->language->id;
		...
		if(get_class($this->context->controller)=="CategoryController") {
			$idCat=(int)Tools::getValue('id_category');
				...
				print_r($result);
				echo "idCat=".$idCat;
				$sqlCat="SELECT id_opartslideshow_slideshow WHERE id_category=".$idCat;
				$slideCat = Db::getInstance()->ExecuteS($sqlSlide);
				foreach($slideCat as $idSlide)  {
					if($result['id_opartslideshow_slideshow']==$idSlide)
						$slides[$result['id_opartslideshow_slideshow']]=$result;
				}
				if($result['id_category']==$idCat)
					$slides[$result['id_opartslideshow_slideshow']]=$result;
